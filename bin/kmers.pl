...
#  -- kmer_merge.pl		 	==>  merges two or more frequency matrices into one bigger one, can be done with either a combined matrix
#  -- kmer_coverage.pl 	 	==>  Estimates the breadth coverage of the genome at multiple confidence tiers, given a set of kmers and their binding sites.
#  -- kmer_search_sites.pl	==>  Searches a range of binding sites and retrieves a set of kmers and ALL their binding sites. 
#  -- kmer_K50.pl     		==>  Computes the K50 score for a given set of kmers and binding sites, informing us which kmers contribute more to the overall utility of a set of kmers.

# PLUGINS IN PROGRESS:
# *-- kmer_statistics.pl 	==> computes scoring and statistics based on the detailed profile generated by kmers.pl.
# *-- kmer_sort.pl       	==> Sorts a file in lexicographic order, when the first element in the row is the kmer name.  (Uses standard Perl sort, takes N*logN time).
# *-- kmer_sites2bed.pl		==> convert a binding sites file  generated by many of these scripts into a BED file
...
sub usage {
	...
	-count_only 		INT flag; only output the counts of each kmer, not the full profile? Default: OFF  
...
$count_only = ( $count_only && $count_only == 1 )? 1 : 0;
...
foreach my $genome_file ( @proc_order )	{
	...
	if ( $count_only == 1 )	{
		open PROFILE, ">", "$genome.kmers.count.tab";
		print PROFILE "Kmer\tCount\n";
	}
	else	{
